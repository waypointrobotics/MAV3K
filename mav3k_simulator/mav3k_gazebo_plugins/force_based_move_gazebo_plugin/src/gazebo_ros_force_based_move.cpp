...
#include <force_based_move_gazebo_plugin/gazebo_ros_force_based_move.h>
                                   ...
                                   sdf::ElementPtr sdf)
{
  ...
  ROS_INFO_STREAM("ForceBasedMove using gains: yaw: " << torque_yaw_velocity_p_gain_ << " x: " << force_x_velocity_p_gain_ << " y: " << force_y_velocity_p_gain_ << "\n");
...
tf::Transform GazeboRosForceBasedMove::getTransformForMotion(double linear_vel_x, double linear_vel_y, double angular_vel, double timeSeconds) const
{
  ...
  {
    tmp.setOrigin(tf::Vector3(linear_x, linear_y, 0.0));
  }
  else
  {
